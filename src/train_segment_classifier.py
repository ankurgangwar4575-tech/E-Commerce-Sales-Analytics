...
def train_segment_classifier():
    print("Loading data for Segment Classification...")
    data_path = Path("data/ecommerce_sales_customer_analytics_150k.csv")
    df = pd.read_csv(data_path)
    
    cat_features = ['gender', 'region']
    num_features = ['customer_age', 'discount_amount', 'gross_sales', 'shipping_cost']
    ...
    with open(model_dir / "segment_classifier_v1_metadata.json", "w") as f:
        json.dump(metadata, f, indent=4)
        
    print("Segment Classifier model training complete!")
